...
export const PlaylistDropdown = ({
  ...
  >
    <div className="flex items-center justify-between px-4 py-2">
      ...
      <button onClick={onRefresh} className="text-xs text-accent-light hover:text-accent transition-colors font-medium">Refresh</button>
    </div>
    <div className="max-h-48 overflow-y-auto" style={{ scrollbarWidth: 'thin', scrollbarColor: 'var(--color-border-default) transparent' }}>
      {playlists.length > 0 ? playlists.map(p => (
        ...
        >
          <ListMusic className="w-4 h-4 text-accent-light shrink-0" />
